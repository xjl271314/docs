...
1. 没法捕捉`try,catch块`，当前代码块有语法错误，JS 解释器压根都不会执行当前这个代码块，所以也就没办法被`catch`住；
...
但它基于`LZ78压缩`，如果后端不支持解压，可选择 gzip 压缩，一般而言后端会默认预装 gzip，因此，选择 gzip 压缩数据也可以，工具包 pako 中自带了 gzip 压缩，可以尝试使用。
...
这里还有一篇采用录屏方式进行捕获的文章[详细地址](https://juejin.cn/post/6953533236337197070?utm_source=gold_browser_extension);
