...
> 如果文本消息仅是文字的话，直接渲染 'xxxx'文字即可，如果文本中带有类似 emoji 的表情，则需要解析表情
...
其中 then 的回调函数参数为 `IMResponse`，`IMResponse.data`属性值如下表所示：
